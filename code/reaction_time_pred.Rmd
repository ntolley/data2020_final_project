...
reach_binary_df <- subset(pred_df, select=-c(reach_time, reaction_time, reach_binary))
...
# Backward and forward stepwise regression
reach_step <- stepAIC(reach_lm, direction = "both")
summary(reach_step)
```
...
```{r}
results_plot <- dwplot(list(reaction_linear_step, reaction_logit_step), ci = 0.90) + 
  labs(x = "Coefficient Estimate", y = "Covariate") + 
  scale_color_discrete(name = 'Model', labels = c('Linear (Reaction)', 'Logistic (Reaction)')) + 
  theme_bw()
